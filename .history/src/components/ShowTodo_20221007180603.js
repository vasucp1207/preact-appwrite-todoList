        ...
        <div>
            {goals.value.map((goal) => (
                <div class='goals-cont'>
                    {completeGoal.value == 'true'? <strike class='strike'><p class='goal'>{goal.content}</p></strike>: <p class='goal'>{goal.content}</p>}
                    <button class='goal-btn' onClick={() => removeGoal(goal.$id)}>❌</button>
                    <button class='goal-btn' onClick={() => doneGoal(goal.$id)}>✔️</button>
                </div>
            ))}
        </div>
    )
